...
function action_send_mobile_code ()
{
	...
	$content = "您正在申请手机注册，验证码为：".$mobile_code."，5分钟内有效！";
	/* 发送激活验证邮件 */
	// $result = true;
	// $result = sendSMS($mobile_phone, $content);
	$resultaaa = duanxin($mobile_phone,$mobile_code, $content);
	// var_dump($resultaaa);exit;
	if($resultaaa)
	{
...
function duanxin($mobile_phone,$mobile_code, $content){
	    $url = 'http://admin.huohuotuan.cn/Verification/loginvalidate';
        $phone = $mobile_phone;
        $senttype = 'SMS_126585512';
        $data = array('phone'=>$phone,'code'=>$mobile_code,'senttype'=>$senttype);
	    $ch = curl_init ();
        // 设置目标服务器
