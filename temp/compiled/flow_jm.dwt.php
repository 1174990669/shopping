...
?>
  ...
  <?php $_from = $this->_var['consignee_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('sn', 'consignee_0_17476900_1520388555');if (count($_from)):
    foreach ($_from AS $this->_var['sn'] => $this->_var['consignee_0_17476900_1520388555']):
?>
...
?>
        ...
        <tr>
          <td colspan=4 class="tdother" id="supplier_shipping"></td>
        </tr>
          <tr>
            <td colspan=4 class="tdother2"><div class="checkout_other" > <a class="jmbag" href="javascript:void(0);" onclick="showCheckoutOther(this);"><span>+</span>使用包装和贺卡</a>
                <table class="subbox_other" width="100%">
                  <tr>
                    <td  align=right width="80">使用包装：</td>
                    <td><select name="pack" style="width:300px;height:25px;" onchange="selectPack(this)">
                        <option value="0" <?php if ($this->_var['order']['pack_id'] == 0): ?>selected<?php endif; ?> >不使用包装</option>
...
?>
                        ...
                        <option value="<?php echo $this->_var['pack']['pack_id']; ?>" <?php if ($this->_var['order']['pack_id'] == $this->_var['pack']['pack_id']): ?>selected<?php endif; ?> ><?php echo $this->_var['pack']['pack_name']; ?>(<?php echo $this->_var['pack']['format_pack_fee']; ?> 免费额度：<?php echo $this->_var['pack']['format_free_money']; ?>)</option>
                        
                        <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
                        
                      </select>
                      <a class="a_other1" href="javascript:void(0);" target="_blank" id="packimage">查看包装图片效果>></a></td>
                  </tr>
                  <tr>
                    <td  align=right>使用贺卡：</td>
                    <td><select name="card" class="otherinput" onchange="selectCard(this)">
                        <option value="0" <?php if ($this->_var['order']['card_id'] == 0): ?>selected<?php endif; ?> ><?php echo $this->_var['lang']['no_card']; ?></option>
...
?>
                        <option value="<?php echo $this->_var['card']['card_id']; ?>" <?php if ($this->_var['order']['card_id'] == $this->_var['card']['card_id']): ?>selected<?php endif; ?> ><?php echo $this->_var['card']['card_name']; ?>(<?php echo $this->_var['card']['format_card_fee']; ?> 免费额度：<?php echo $this->_var['card']['format_free_money']; ?>)</option>
                        <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
                      </select>
                      <a class="a_other1" href="javascript:void(0);" target="_blank" id="cardimage">查看贺卡图片效果>></a></td>
                  </tr>
                  <tr>
                    <td  align=right valign=top>祝福语：</td>
                    <td><textarea rows=5 cols=70 disabled=disabled name="card_message" id="card_message"><?php echo htmlspecialchars($this->_var['order']['card_message']); ?></textarea></td>
                  </tr>
                </table>
              </div></td>
          </tr>
		...
		packimage[0]="javascript:void(0);";
		cardimage[0]="javascript:void(0);";
		<?php $_from = $this->_var['pack_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'pack');if (count($_from)):
    foreach ($_from AS $this->_var['pack']):
?>
		<?php if ($this->_var['pack']['pack_img']): ?>
		packimage[<?php echo $this->_var['pack']['pack_id']; ?>]="data/packimg/<?php echo $this->_var['pack']['pack_img']; ?>";
			<?php if ($this->_var['order']['pack_id'] == $this->_var['pack']['pack_id']): ?>
			document.getElementById('packimage').href="data/packimg/<?php echo $this->_var['pack']['pack_img']; ?>";
			<?php endif; ?>
		<?php else: ?>
		packimage[<?php echo $this->_var['pack']['pack_id']; ?>]="javascript:void(0);";
		<?php endif; ?>
		<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
		<?php $_from = $this->_var['card_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'card');if (count($_from)):
    foreach ($_from AS $this->_var['card']):
?>
		...
		cardimage[<?php echo $this->_var['card']['card_id']; ?>]="data/cardimg/<?php echo $this->_var['card']['card_img']; ?>";
			<?php if ($this->_var['order']['card_id'] == $this->_var['card']['card_id']): ?>
			document.getElementById('cardimage').href="data/cardimg/<?php echo $this->_var['card']['card_img']; ?>";
			document.getElementById('card_message').disabled=false;
			<?php endif; ?>
		<?php else: ?>
		cardimage[<?php echo $this->_var['card']['card_id']; ?>]="javascript:void(0);";
		<?php endif; ?>
		<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
    ...
    <div class="blank10"></div>
          <tr>
            ...
            <td colspan=4 class="tdother2"><?php if ($this->_var['inv_content_list'] || $this->_var['how_oos_list']): ?>
